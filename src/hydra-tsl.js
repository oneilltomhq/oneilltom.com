...
// fractional Brownian motion over a vec2 node, built from mx_noise remapped to
// [0,1]. Five octaves, unrolled at compile time (JS loop → flat node graph, no
// GPU loop). The domain-warp primitive behind the `warp` source.
const fbm2 = ( p ) => {
...
const defs = {
	...
	// domain-warped fbm nebula — the "synth" backdrop look ported from the
	// design system's SynthStage fragment shader. q = fbm(p+t),
	// r = fbm(p+2.2q+offsets±t), f = fbm(p+3r); dark base, warm/cool mid,
	// pink filaments gated toward the right (uv.x) so text stays calm on the
	// left. The fbm loop unrolls at compile time (no GPU loop). Params are
	// scalar uniforms → live-tweakable, so each preset differs only by
	// scale / warm / cool / pink / seed / speed, exactly like the reference.
	warp: { type: 'src', defaults: [ 3.2, 0.34, 0.72, 0.5, 21, 1 ],
		fn: ( st, scale, warm, cool, pink, seed, speed ) => {
			...
			col = col.add( vec3( 1.0, 0.72, 0.92 ).mul( fil ).mul( pink )
				.mul( smoothstep( 0.3, 1.0, st.x ) ) );
			// left→right brightness ramp (text calm on the left) + vignette —
			// these dim the field and give it depth, and are what keep the
			// reference from reading as a flat lavender wash.
			col = col.mul( mix( 0.5, 1.15, smoothstep( 0, 1, st.x ) ) );
